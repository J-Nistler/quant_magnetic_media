...
## What are the major patterns of variation amongst formats?



Below, we see that most variation (PC1) separates a continuum of formats, 
with common formats like VHS and Compact Cassette towards the right along with
less common formats like Video 8, DAT, and DVCAM. This may also reflect that
most less common formats may be noisily distributed, and by chance have patterns
near or distinct from the common formats.

```{r pca_media,cache=T}
pca_media <- prcomp(t(matrix_by_media[apply(matrix_by_media,1,sum)>0,]),
    center=T,scale=T)
...
The second component of variation separates the rare formats Microcassette, D3,
D2, F-1, and Digital 8. It appears this variation may be driven by presence
in the Nebraska collection, with one large and comprehensive collection 
dwarfing the contributions of other collections.

```{r}
tdatar %>% filter(
        FormatName %in% c(
            "Microcassette","Digital 8","D3","D2","F-1"
            ) &
        !is.na(EstItemCount)
        )%>%
    select(State,FormatName,EstItemCount) %>%
    arrange(FormatName,State) %>%
    data.frame()
```

In the this, and in other components of variation of format distribution, it
is likely that these apparent patterns are driven by noise in the sampling.
Filtering and analyzing common formats may yield more interesting patterns.
