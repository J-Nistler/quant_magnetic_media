...
We proceeded to analyze all responses together, and later discuss apparent
differences in collection composition.
...
Self-assessment of skill might reflect how the archivist perceives the 
institution type, and so we compared self-assessed skill against different types
of respondents.
Since very few respondents that were not from "Academic"
institutions, no conclusive pattern emerges.

```{r skill_by_instutition_type,cache=T}
all_responses %>%
    filter(!is.na(PracticeSelfAssesment)) %>%
    {
    ggplot(.)+theme_bw()+
    aes(x=InstitutionType,y=PracticeSelfAssesment)+
    geom_point(aes(size=..count..),stat="bin_2d")+
    scale_x_discrete(limits=names(sort(table(.$InstitutionType),decreasing=T)))+
#    geom_label(aes(label=..count..,y=..count..),stat="count")+
    ...
    scale_size_continuous(breaks=seq(0,12,4))+
    guides(fill=F)+
    xlab("Institution type")+
    ylab("Self-assessed skill")
    }
```

Self-assessment is unreliable without criteria.
Developing a standardized set of practices around these media formats would help
archivists self-identify shortcomings and correct it themselves, given adequate
support from their institution.

## Respondents are geographically dispersed 

The following map shows respondent count per state in white circles.
Hue corresponds to average temperatures (HOW WAS THIS DEFINED I FORGOT),
while intensity corresponds to total collection size.

We were satisfied to see that we were able to reach institutions across the
United States, but we do see a high concentration in New York and Oregon. 

The temperature icons in Utah and Nebraska indicate two examples of large
collections maintained in "room-temperature" conditions in warm climates, and
are discussed more below.

<img style="width:100%;" 
    src="img/choropleth_magnetic_media_survey_2019.png"/>
...
Tools like the AMIA Video Preservation Fact Sheet are important for sharing
institutional knowledge across the broader community, and surveys like this
can help disseminate these tools.
We are optimistic that this dataset will allow archivists with poor-quality 
storage facilities to advocate for the support they need to bring their 
collection to meet the average practice.
...
Each y-axis is in log-scale, and so differences are exponential.
...
g_items_per <- tdatar %>% 
    ...
    theme(axis.text.x=element_text(angle=90))+
    ylab("Each respondent's estimated\nitems per format")+
    xlab("")
    }
g_items_per
```
...
g_items_prop <- tdatar %>% 
    ...
    scale_y_log10(labels=function(x){formatC(x,big.mark=",",format="d")},
        limits=c(0.9,NA),breaks=c(1,10,25,51,100))+
    scale_x_discrete(
        limits=group_by(.,FormatName)%>%
            summarize(z=sum(EstItemCount,na.rm=T))%>%
            arrange(-z)%>%pull(FormatName)
    ...
    theme(axis.text.x=element_text(angle=90))+
    ylab("The percentage of each collection\nthat is each format")+
    xlab("")
    }
g_items_prop
```
...
To summarize the collection compositions across institutions and formats,
we display them as a heatmap below.
The blue color key denotes the total estimated items of that format in that
respondent's collection, on a logarithmic scale (exponentially increasing).
Format type is each column, labelled at the bottom. 
Each respondent is a row, with the estimated percentage of completion labelled.

Hierarchical clustering is a method to compare columns or rows to each other,
such that adjacent rows are more similar than distant rows. When this is done,
a tree is drawn along the side that is clustered, denoting how "related" the
rows or columns are to each other. 


```{r heatmap_data,cache=T}
...
The color scale at the top denotes the risk category.

```{r risk_key,fig.height=2,fig.width=2,cache=T}
...
```{r heatmapz_1,fig.height=9,fig.width=9,cache=T}
making_that_map(dendrogram="both",
    margins=c(08,7),
    ylab="Respondent, labelled with\npercentage of estimated completion")
```


We qualitatively identify that VHS, open reel audio, and compact cassettes are
one group of abundant and commonly held items, and appear to be the bulk of
many collections. Similarly, U-Matic and Betacam formats are similar, and 
together with Open Reel Video and MiniDV form another cluster of somewhat
common formats with less abundance but higher risk.

It appears that VHS, Compact Cassette, and Open Reel Audio are maybe the most 
commonly encountered cluster of formats. However, the higher risk Betacam and 
U-matic, and perhaps Open Reel Video with MiniDV, could be grouped for 
training, programming, and equipment in preserving these common but higher risk
formats.
...
While the data comes from a limited pool of academic archives and therefore
cannot speak for all archives, the data can still serve to indicate what may be
expected. Additional research addressing the preservation status of magnetic
media collections in academic libraries, and to what degree are they
endangered, could only make a stronger case for funding proposals. However,
even without further research, it is imperative to report that archivists must
begin by conducting assessments and evaluations of their collections in order
to allocate resources to digitize their magnetic media collections. This is a
crucial first step that needs to happen now, not later during the institution’s
five-year plan. Moreover, this study illustrates that many institutions have
not taken this first step.

# Future Opportunities
- Extend the questionnaire to more institutions for a diverse population of archives. Our initial call asked for academic institutions, but about 16% of submissions came from public or private companies, archives, historical societies, and museums.
- Many funders support digitization and full preservation of AV magnetic materials. However, there are significatn barriers to entry. Applying for the grants is a time-intensive process (e.g. an archive compiled 221 pages of proof for a $20,500 grant) in order for institutions to receive a grant from $5k to $50k (CLIR 209). This grant does not guarantee administrative labor to be funded, as seen in the requirement that the digitization process cannot happen in-house, which makes employment and institutional dedication difficult to maintain when the work is being done off-site and short-term.
- Older institutions tend to deviate from core fundamentals of archival practices. Backlogs will get bigger than capable of processing. How do we pause acquisitions to catch up?
- Compare data to OCLC's recent publication _Assessing Needs of AV in Special Collections_ by Chela S. Weber.
- Compare data to the soon-to-be published study on obsolescence of technical expertise within the field of moving image archiving and preservation by Brianna Toth. This will help us understand which playback decks/digitization transfer stations to prioritize because finding functioning machines, maintaining the machines, and finding knowledgeable personnel is a challenge within itself.

# Acknowledgements

Many thanks to: our LIS 570 collaborators, Brian Click and Michael Kuster, who
helped with the literature review, method design, and co-writing the original
paper. Paul J. Weiss, LIS 570 instructor, who inspired and guided us to use
this project to engage and contribute to the larger archives community. All
survey respondents, interviewees, and archive listervs who welcomed our
research to the discussion forums. Thank you SAA for accepted our research to
...
## Do different institutions have patterns of collection composition?

Looking between institutions, how are collections composed?
Are there major patterns of the differences between institutions?

We can use principal components analysis to pull out the major ways in which
formats are distributed across institution. Principal components are ranked in 
order of their magnitude, so 1 explains the most variation while 2 explains
the next most amount of variation. Counts were scaled and centered before being
...
pdatar <- pca_inst$rotation %>% 
    ...
    gather(variable,value,starts_with("Percentage_")) %>% 
...
Do institution types have different patterns of composition?
In the two largest axes of variation, this is not apparent, but in axes
3 and 4 we see that museums and the only historical society are well seperated.

```{r pca_inst_1,cache=T}
pc12+aes(fill=InstitutionType)
pc34+aes(fill=InstitutionType)
```

What differences in formats underlie these patters?
Why are museums different than academic archives?

We correlated the counts of each format with the position on each axes of 
variation. Below these are plotted, and the labelled points are the correlations
that were significantly correlated (spearman) by p < 0.05 with a bonferroni
...
ggplot(rez)+theme_bw()+
    ...
    facet_wrap(~PC)+
    geom_point()+
    geom_label_repel(data=subset(rez,pval<(0.05)/6))
```

We see that Open Reel Audio and Compact Cassette are both well correlated with
the two largest axes of variation, and may be indicative of large patterns of
variation in collection formats

```{r replot_openreelandcassette,cache=T}
openreelandcassette <- inst_pca_plot+
    aes(fill=0+
        as.numeric(sub("%","",`Percentage_Open Reel Audio`))+
        as.numeric(sub("%","",`Percentage_Compact Cassette`))
        )+
    scale_fill_distiller("openreelandcassette",palette="Spectral",na.value="white")
openreelandcassette+aes(x=PC1,y=PC2)+
    xlab(paste0("PC1 ",percent_varz[1],"% variance"))+
    ylab(paste0("PC2 ",percent_varz[2],"% variance"))
```

On the third and fourth largest axes of variation, we examine how Betacam, 
Umatic, and Other formats are distributed, and find that there are two clusters
of large Betacam or Umatic collections amongst Academic institutions and that
the museums and historical society have more Other format media (compare to
plots above).
...
pc34+aes(fill=0+
    ...
    scale_fill_distiller("Betacam or Umatic",palette="Spectral",
        na.value="white")

pc34+aes(fill=0+
        as.numeric(sub("%","",`Percentage_Other`))
        )+
    scale_fill_distiller("Other",palette="Spectral",
        na.value="white")
```

All together, this suggests that Betacam and Umatic are distinctly varying
formats and so we may see heterogenity amongst institutions in holding these
formats.
The Other category distinguishes museums and the historical society that 
responded, suggesting that these institutions have distinct formats not 
addressed or categorized here.


<!--- supplemental making figures for poster, etc -->
...
ggsave(paste0(fig_dir,"items_per_respondent.png"),g_items_per,width=6,height=5)
ggsave(paste0(fig_dir,"items_per_respondent.svg"),g_items_per,width=6,height=5)
...
```{r heatmapz_1_save,fig.height=9,fig.width=9,cache=T,echo=F}
...
```{r heatmapz_2,fig.height=9,fig.width=9,cache=T,echo=F}
svg("magnetic_heatmap_ordered_by_chance_completion_at_top.svg",width=6,height=6)
making_that_map(dendrogram="col",Rowv=F,
    margins=c(08,5),
...
(See original paper for the full bibliography [https://github.com/J-Nistler/quant_magnetic_media/blob/master/NguyenNistlerClickKuster_MagneticMediaSurvey_F2018.pdf])
