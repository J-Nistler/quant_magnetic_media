...
use a pandoc template (already existing?) to put abstract with title up top
...
```{r,cache=T,echo=F}
# Here's loading libraries, that other kind of library
library(tidyverse)
library(ggrepel)
...
raw_datar <- read_csv("survey_data_2019.csv",
    col_names=c(
        ...
        "Unstructured_StorageConditions",
...
responses_with_totals <- distinct(filter(raw_datar,!is.na(raw_datar$TotalItems)))
...
We recieved ```r nrow(raw_datar)``` responses
