...
```{r responses_self_versus_completion,cache=T}
all_responses %>%
    filter(!is.na(PracticeSelfAssesment)) %>%
    filter(!is.na(PercentDigitizedBeforeUnplayable)) %>%
    {
    ggplot(.)+theme_bw()+
    aes(x=PracticeSelfAssesment,y=as.numeric(PercentDigitizedBeforeUnplayable))+
    stat_bin2d(geom="point",aes(size=..count..),shape=15,binwidth=c(1,20))+
    guides(fill=F)+
    scale_x_discrete(limits=c(
            "Excellent",
            "Above Average",
    ...
    theme(axis.text.x=element_text(angle=90))+
    xlab("Self-assessment of archival skillz")+
    ylab("Self-assessed estimate of complete digitization\nbefore media is unplayable (binned into fifths)")
    }
```

## Total itemss of each format type and their obsolescence risk factor
** POSTER-2**
```{r format_counts_divided_x_by_risk,cache=T,fig.height=7}
tdatar %>% 
    ggplot()+theme_bw()+
    aes(x=Risk,group=FormatName,weight=EstItemCount,label=FormatName)+
    geom_point(stat="count")+
    geom_label_repel(stat="count")+
    theme(axis.text.x=element_text(angle=90))+
    ylab("Total estimated items across survey")+
    xlab("Risk Estimate")
```
TO DO: no label borders and move dots to not overlap

Description: Basic map of qty vs risk
...
tdatar %>% 
    ...
    scale_y_log10()+
    scale_x_discrete(limits=group_by(.,FormatName)%>%summarize(z=sum(EstItemCount,na.rm=T))%>%arrange(-z)%>%pull(FormatName))+
    theme(axis.text.x=element_text(angle=90))+
    ylab("Total estimated items across survey")+
    xlab("Format")
    }
...
**DELET BELOW**
Notice this one is cropping out one observation of a fraction of a fraction of
a percentage of "Other" media.
```{r percentage_of_collections_that_are_each_format,cache=T}
# Notice this one is cropping out one observation of a fraction of a fraction of
# a percentage of "Other" media.
tdatar %>% 
    {
    ...
    scale_x_discrete(limits=group_by(.,FormatName)%>%summarize(z=sum(Percentage,na.rm=T))%>%arrange(-z)%>%pull(FormatName))+
    theme(axis.text.x=element_text(angle=90))+
    ylab("Percentage of collection that is that item")+
    xlab("Format")
    }
```
